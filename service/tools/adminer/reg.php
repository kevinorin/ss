	...
	if (isset($_POST['type']) && isset($_POST['pattern'])) {
		...
		function wtMatchSpan($m) {
			...
			for ($i=1;$i<=$s;$i++) {
				 ...
				 $l=strlen($m[$i]);
				 if ($p!==false) {
						$r.=substr($b,0,$p)."<span class='matches$i' title='$i'>".$m[$i]."</span>";
						$b=substr($b,$p+$l);
				 }
			...
			if (!$r) $r=$b;
			return "<span class='matches'>$r</span>";
		}
...
<!DOCTYPE html><html><head><head>
		...
		<title>WT-NMP Regular Expression Tester</title>
...
#code {
 ...
 dl { font-size:small; }
 dt { float: left; clear: left; text-align: right; font-weight: bold; color: green; }
/* dt:after { content: ":"; }padding: 0 0 0.5em 0;*/
		...
		<script type="text/javascript">
			...
			<div id="main">
				...
				<ul class="tabs" id="menu_tabs">
					...
					<li style="float:right;"><a href="http://wtriple.com/wtnmp?wtSrc=reg">WT-NMP</a></li>
				</ul>
				</div>
				<div class="evaluator">
					<p><b>Pattern</b> (start and end slashes will be added when needed)</p>
